...
import (
	"context"
	"fmt"
	"github.com/let-sh/cli/handler/dev/remotedialer"
...
func (c *Client) Connect() error {
	headers := http.Header{}
	headers.Set("x-letsh-tunnel-id", uuid.Formatter(uuid.NewV4(), uuid.FormatHex))
	for k, v := range c.UpstreamMap {
		headers.Add("x-letsh-upstream", fmt.Sprintf("%s=%s", k, v))
	}
	if c.Token != "" {
